...
router.get('/', async (req, res) => {
  try {
    const { data, error } = await req.supabase.from('persons').select('*, person_addresses(*), events(*)').order('created_at', { ascending: false });
...
router.get('/:id', async (req, res) => {
  try {
    const { data, error } = await req.supabase.from('persons').select('*, person_addresses(*), events(*, event_occurrences(*))').eq('id', req.params.id).single();
    if (error) throw error;
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete('/:id', async (req, res) => {
  try {
    const { error } = await req.supabase.from('persons').delete().eq('id', req.params.id);
    if (error) throw error;
